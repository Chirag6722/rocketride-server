...
def _safe_extract(archive: Path, dest: Path) -> None:
    ...
    if archive.name.endswith('.zip'):
        ...
        with tarfile.open(archive) as t:
            if any(os.path.isabs(m.name) or '..' in Path(m.name).parts for m in t.getmembers()):
                raise ValueError('unsafe archive member')
            t.extractall(dest)
    ...
    with _lock:
        ...
        try:
            proc = subprocess.Popen(
                [str(binary)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, cwd=str(binary.parent)
            )
        except OSError as e:
