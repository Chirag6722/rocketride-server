...
- **`tool_github` node**: added a generic `request` tool and tool-group gating, so an agent is no longer limited to the 37 hand-wired operations. `request` reaches any GitHub REST endpoint by method and path (gists, projects, teams, deployments, checks, packages, branch protection and the rest) through the same auth, rate-limit and read-only layer, gated by `github.allowRawRequest` (default on). `github.toolGroups` selects which of the 8 groups (`files`, `issues`, `pull_requests`, `repos`, `releases`, `workflows`, `search`, `org_members`) are published; **leaving it empty publishes all 37 tools, which is the existing behaviour**, so upgrading changes nothing until the field is set. **Read-only mode now hides the 17 write tools instead of refusing them at invoke time**: an agent cannot tell in advance that a published tool is blocked, so it spent a turn finding out. `request` stays published under read-only and accepts GET only (#1364)

### Fixed

- **`tool_github` node**: `github_client.call()` no longer raises a fatal error on a rate limit or a missing file. `429`, and `403` responses that are demonstrably rate limits, are now retried using GitHub's own `retry-after` and `x-ratelimit-reset` headers; a `403` naming a permission, SSO or scope problem is never retried, since it is permanent. Retries are bounded at 3 attempts, 60s per sleep and 90s per call, so an exhausted core budget (which can reset up to an hour out) fails immediately with the reset time in the message rather than blocking a pipeline thread. `file_get` now returns `found: false` for a 404 instead of ending the run, and says plainly that GitHub answers 404 identically for a missing file, an invisible repository and an unknown ref. Agent-supplied paths are validated before the request is built: a path such as `@evil.com/x` previously resolved to a different host and would have sent the token with it. Worst-case latency for a single call rises from 30s to 180s as a result of the retry budget (#1352)
